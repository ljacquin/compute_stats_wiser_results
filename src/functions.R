...
simulate_experimental_design <- function(
    n_env = 10,
    n_geno = 30, n_rep = 5) {
  # create a list of environments
  env_list_ <- paste0("environment_", 1:n_env)

  # create a list of genotypes
  geno_list_ <- paste0("genotype_", 1:n_geno)

  # generate data frame
  df_ <- expand.grid(Environment = env_list_, Genotype = geno_list_)
  df_ <- df_[rep(seq_len(nrow(df_)), each = n_rep), ]

  # shuffle data to vary order
  df_ <- df_[sample(nrow(df_)), ]
...
simulate_experimental_design <- function(
  ...
  # Assign random repetitions between 2 and n_rep for each genotype in each environment
  df_$Reps <- sample(2:n_rep, size = nrow(df_), replace = TRUE)

  # Expand rows based on repetitions
  df_ <- df_[rep(seq_len(nrow(df_)), df_$Reps), c("Environment", "Genotype")]
...
simulate_experimental_design <- function(
    ...
    min_n_rep = 2,
    max_n_rep = 5,
    control_genotypes = c("genotype_1", "genotype_2"), # specify control genotypes
    control_multiplier = 2, # factor to increase repetitions for control genotypes
    missing_rate = 0.2, # proportion of genotypes to remove per environment
    apply_missing_rate = FALSE,
    collinearity_strength = 0.1 # strength of genotype-environment correlation
    ) {
  # create a list of environments
  env_list_ <- paste0("environment_", 1:n_env)
  ...
  # assign random repetitions between min_n_rep and max_n_rep
  df_$Reps <- sample(min_n_rep:max_n_rep, size = nrow(df_), replace = TRUE)

  # increase repetitions for control genotypes
  df_$Reps[df_$Genotype %in% control_genotypes] <- df_$Reps[
    df_$Genotype %in% control_genotypes
  ] * control_multiplier

  # artificially remove some genotypes in certain environments to create confusion
  if (apply_missing_rate) {
    for (env in env_list_) {
      missing_genotypes <- sample(geno_list_, size = round(n_geno * missing_rate))
      df_ <- df_[!(df_$Environment == env & df_$Genotype %in% missing_genotypes), ]
    }
  }

  # introduce strong multicollinearity by forcing certain genotypes to appear more
  # often in specific environments
  for (i in seq_len(n_env)) {
    if (runif(1) < collinearity_strength) {
      env <- env_list_[i]
      preferred_genotypes <- sample(geno_list_, size = round(n_geno * collinearity_strength))
      df_$Reps[df_$Environment == env & df_$Genotype %in% preferred_genotypes] <- max_n_rep
      df_$Reps[df_$Environment == env & !df_$Genotype %in% preferred_genotypes] <- min_n_rep
    }
  }
...
# get contingency genotype x site contigency table for apple data
create_contingency_table <- function(df_, var_1, var_2) {
  table_df <- as.data.frame.matrix(table(df_[, var_1], df_[, var_2]))
  return(table_df)
}
...
# function which computes repeatability
compute_R <- function(df_) {
  # verify if genotype column is in the dataframe
  if (!"Genotype" %in% colnames(df_)) {
    stop("Genotype column not in dataframe.")
  }
  ...
  mean_Y <- mean(df_long$Value, na.rm = TRUE)

  # compute mean by genotype
  mean_by_genotype <- df_long %>%
    group_by(Genotype) %>%
    summarise(mean_Yi = mean(Value, na.rm = TRUE), .groups = "drop")
  ...
  btw_ssq <- sum(ni$n * (mean_by_genotype$mean_Yi - mean_Y)^2)

  # compute total sum of squares
  ssq_total <- sum((df_long$Value - mean_Y)^2)
...
# function to calculate the orthogonal distance from a point (x0, y0) to the
# regression line y = mx + c
orthogonal_distance <- function(x0, y0, m, c) {
...
# function which computes and format IQR
format_median_iqr <- function(x) {
  med <- round(median(x, na.rm = TRUE), 2)
  iqr_ <- round(IQR(x, na.rm = TRUE), 2)
  return(paste0(med, " (", iqr_, ")"))
}
